...
type SENTIMENT = '高兴' | '无情感' | '喜欢' | '悲伤' | '厌恶' | '愤怒';

export interface ResponseSENTIMENT {
  error: null;
  result: [[keyof SENTIMENT, number]];
}

type SENSITIVE = 'dirty_curse' | 'other' | 'dirty_politics' | 'dirty_porno';
export interface ResponseSENSITIVE {
  error: null;
  result: [[keyof SENSITIVE, number]];
}
